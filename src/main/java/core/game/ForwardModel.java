...
public class ForwardModel extends Game {
	...
	public ForwardModel(Game a_gameState) {
		// All static elements of the game are assigned from the game we create
		// the copy from.
		initNonVolatile(a_gameState);

		// Init those variables that take a determined value at the beginning of
		// a game.
		init();
	}
	...
	public final void update(Game a_gameState)
    {
        ...
        for(int i = 0; i < spriteGroups.length; ++i)
        {
            ...
            if(null != spriteIt) while(spriteIt.hasNext())
            {
        ...
        //events:
        historicEvents.addAll(a_gameState.historicEvents.stream().map(Event::copy).collect(Collectors.toList()));
	 ...
	 */
	private void updateGrid(Observation obs, boolean newObs, boolean moved,
	...
	private void removeObservationFromGrid(Observation obs, Vector2d position) {
		...
		boolean validX = 0 <= x && x < observationGrid.length;
		boolean xPlus = 0 < position.x % block_size
				&& x + 1 < observationGrid.length;
		int y = (int) position.y / block_size;
		boolean validY = 0 <= y && y < observationGrid[0].length;
		boolean yPlus = 0 < position.y % block_size
				&& y + 1 < observationGrid[0].length;
	...
	public void printObservationGrid() {
		...
		for (int j = 0; j < observationGrid[0].length; ++j) {
			for (ArrayList<Observation>[] anObservationGrid : observationGrid) {
				int n = anObservationGrid[j].size();
				if (0 < n)
					System.out.print(n);
				else
					System.out.print(' ');
	...
	private Observation createSpriteObservation(VGDLSprite sprite) {
		...
		Observation obs = new Observation(sprite.getType(), sprite.spriteID,
				sprite.getPosition(), Types.NIL, category);
	...
	private Observation getSpriteObservation(VGDLSprite sprite) {
		int spriteId = sprite.spriteID;
		return observations.containsKey(spriteId)
				? observations.get(spriteId)
				: createSpriteObservation(sprite);
	}
	...
	private void initNonVolatile(Game a_gameState) {
		...
		score = a_gameState.score;
		frame_rate = a_gameState.frame_rate; // is this needed?
		...
		observationGrid = new ArrayList[screenSize.width / block_size][screenSize.height
				/ block_size];
	 ...
	 * follows the opposite order of the drawing order (spriteOrder[]). It uses
	 * the action received as the action of the avatar.
	 * 
	 * @param action
	...
	protected void tick(Types.ACTIONS action) {
		...
		if (null != avatar)
			avatar.performActiveMovement(Game.ki.getMask());

		// Now, update all others (but avatar).
		int typeIndex = spriteOrder.length - 1;
		for (int i = typeIndex; 0 <= i; --i) // For update, opposite order than
												// drawing.
		{
			int spriteTypeInt = spriteOrder[i];

			Iterator<VGDLSprite> spriteIt = spriteGroups[spriteTypeInt]
					.getSpriteIterator();
			if (null != spriteIt)
	 ...
	 * 
	 * @param groupArray
	 *            boolean map that indicates which sprite types must be
	 *            considered.
	 * @return List of arrays with Observations. Each entry in the array
	 *         corresponds to a different sprite type.
	 */
	private ArrayList<Observation>[] getPositionsFrom(boolean[] groupArray,
			Vector2d refPosition) {
		// First, get how many types we have.
		int numDiffTypes = 0;
		for (int i = 0; i < groupArray.length; ++i) {
			// There is a sprite type we don't know anything about. Need to
			// check.
			if (!knownList[i] && null != spriteGroups[i].getFirstSprite())
		...
		for (int i = 0; i < groupArray.length; ++i) {
			if (groupArray[i]) {
				...
				Iterator<VGDLSprite> spriteIt = spriteGroups[i]
						.getSpriteIterator();
				if (null != spriteIt)
					while (spriteIt.hasNext()) {
						VGDLSprite sp = spriteIt.next();
						Observation observation = getSpriteObservation(sp);
						observation.update(i, sp.spriteID, sp.getPosition(),
								reference, getSpriteCategory(sp));
	 ...
	 * 
	 * @param refPosition
	 *            Reference position to use when sorting this array, by
	 ...
	 */
	public ArrayList<Observation>[] getFromAvatarSpPositions(
			Vector2d refPosition) {
	...
	public void buildLevel(String gamelvl) {
		throw new RuntimeException(
				"buildLevel should not be called in this instance.");
	}

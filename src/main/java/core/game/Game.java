...
/**
 * Created with IntelliJ IDEA. User: Diego Date: 17/10/13 Time: 13:42 This is a Java port from Tom Schaul's VGDL -
...
public abstract class Game {
  ...
  /**
   * Content objects for the different sprite types.. The index is the type of object Content encloses information about
   * the class of the object and its parameters.
   */
  protected Content[] classConst;

  /**
   * Groups of sprites in the level. Each element of the array is a collection of sprites of a given type, which is also
   * the index of the array.
   */
  protected SpriteGroup[] spriteGroups;

  /**
   * Relationships for collisions: double array of (list of) effects. Interaction between two sprites can trigger more
   * than one effect. collisionEffects[] -> int id of the FIRST element taking part on the effects. collisionEffects[][]
   ...
   */
  protected List<VGDLSprite> kill_list;
  ...
  /**
   * Indicates if the game has been won by the player. See Types.WINNER for the values of this variable.
   */
   ...
   */
  public void initSprites(List<Integer> spOrder, Iterable<Integer> sings,
      Map<Integer, SpriteContent> constructors) {
    ...
    for (Map.Entry<Integer, SpriteContent> entry : entries) {
      ...
      if ("Resource".equals(refClass)) {
        VGDLSprite resourceTest =
            VGDLFactory.GetInstance().createSprite(entry.getValue(), new Vector2d(0, 0),
                new Dimension(1, 1));
    ...
    bucketList = new Game.Bucket[classConst.length];
  ...
  /**
   * Adds a new sprite to the pool of sprites of the game. Increments the sprite counter and also modifies is_stochastic
   * and the avatar accordingly.
   * 
  ...
  public int getNumSprites(int itype) {
    int acum = 0;
    for (Integer subtype : iSubTypes[itype]) {
  ...
  public double runGame(AbstractPlayer player, int randomSeed) {
    ...
    while (!isEnded) {
      gameCycle(); // Execute a game cycle.
    }
  ...
  public double playGame(AbstractPlayer player, int randomSeed) {
    ...
    while (!isEnded) {
      ...
      frame.setTitle("Java-VGDL: Score:" + score + ". Tick:" + getGameTick());
    }
  ...
  /**
   * Initializes some variables for the game to be played, such as the game tick, sampleRandom number generator, forward
   * model and assigns the player to the avatar.
   * 
   * @param player Player that plays this game.
   * @param randomSeed sampleRandom seed for the whole game.
   */
  ...
  /**
   * This is a standard game cycle in J-VGDL. It advances the game tick, updates the forward model and rolls an action
   * in all entities, handling collisions and end game situations.
   */
  private void gameCycle() {
    ...
    tick(); // update for all entities.
    eventHandling(); // handle events such collisions.
    clearAll(); // clear all additional data, including dead sprites.
    terminationHandling(); // check for game termination.
    checkTimeOut(); // Check for end of game by time steps.
  ...
  /**
   * Handles the result for the game, considering disqualifications. Prints the result (score, time and winner) and
  ...
  private double handleResult() {
    ...
    if (Types.WINNER.PLAYER_WINS == winner)
      if (0 >= score)
        score = 1;
  ...
  /**
   * Checks if the game must finish because of number of cycles played. This is a value stored in
   * CompetitionParameters.MAX_TIMESTEPS. If the game is due to end, the winner is determined and the flag isEnded is
   * set to true.
   */
  ...
  private void printResult() {
    System.out.println("Result (1->win; 0->lose):" + winner.key() + ", Score:" + score
        + ", timesteps:" + getGameTick());
  }
  ...
  /**
   * Looks for the avatar of the game in the existing sprites. If the player received as a parameter is not null, it is
   * assigned to it.
   * 
  ...
  private void assignPlayer(AbstractPlayer player) {
    ...
    while (null == avatar) {
      ...
      if (0 < spriteGroups[spriteTypeId].numSprites()) {
  ...
  /**
   * Performs one tick for the game: calling update(this) in all sprites. It follows the opposite order of the drawing
   * order (spriteOrder[]).
   */
  protected void tick() {
  ...
  protected void eventHandling()
    {
        ...
        for(Integer intId : definedEOSEffects)
        {
            ...
            {
                if(!noSprites[intId] && 0 == bucketList[intId].size())
                {
                    //Take all the subtypes in the hierarchy of this sprite.
                    Iterable<Integer> allTypes = iSubTypes[intId];
                    for(Integer itype : allTypes)
                    {
                        ...
                        Collection<VGDLSprite> sprites = getSprites(itype).values();
                        //bucketList[intId].insert(sp);
                        sprites.forEach(bucketList[intId]::add);
                    }

                    //If no sprites were added here, mark it in the array.
                    if(0 == bucketList[intId].size())
        ...
        for(Pair p : definedEffects)
        {
            ...
            for(Effect ef : collisionEffects[p.first][p.second])
            {
                ...
                Map<Integer, ArrayList<VGDLSprite>> second = bucketList[p.second].getSpriteList();
                ...
                for(Integer bucket1 : first.keySet())
                {
                    ...
                    Iterable<VGDLSprite> sprites1nBucket1 = first.get(bucket1);

                    //For every sprite:
                    if(null != sprites1nBucket1) for(VGDLSprite s1 : sprites1nBucket1)
                    {
                        //Decide in what buckets to look.
                        int[] buckets = s1.bucketSharp ? new int[]{s1.bucket - 1, s1.bucket} : new int[]{s1.bucket, s1.bucket + 1};


                        for(int bucketId : buckets)
                        {
                            //On each bucket, take the sprites if the p.second sprite type.
                            List<VGDLSprite> spritesInBucket2 = second.get(bucketId);
                            if(null != spritesInBucket2 && !kill_list.contains(s1))
                            {
                                int numSprites2 = spritesInBucket2.size();
                                for (VGDLSprite s2 : spritesInBucket2) {
  ...
  private void addEvent(VGDLSprite s1, VGDLSprite s2) {
    if (s1.is_avatar)
      historicEvents.add(new Event(gameTick, false, s1.getType(), s2.getType(), s1.spriteID,
          s2.spriteID, s1.getPosition()));

    else if (s1.is_from_avatar)
      historicEvents.add(new Event(gameTick, true, s1.getType(), s2.getType(), s1.spriteID,
          s2.spriteID, s1.getPosition()));

    else if (s2.is_avatar)
      historicEvents.add(new Event(gameTick, false, s2.getType(), s1.getType(), s2.spriteID,
          s1.spriteID, s2.getPosition()));

    else if (s2.is_from_avatar)
      historicEvents.add(new Event(gameTick, true, s2.getType(), s1.getType(), s2.spriteID,
          s1.spriteID, s2.getPosition()));
  }
  ...
  public VGDLSprite addSprite(SpriteContent content, Vector2d position, int itype) {
    ...
    for (Integer typeInt : content.itypes) {
      ...
      if (singletons[typeInt] && 0 < getNumSprites(typeInt)) {
    ...
    if (!anyother) {
      VGDLSprite newSprite =
          VGDLFactory.GetInstance().createSprite(content, position,
   ...
   * @return sprite collection of the specified type.
   */
  public SortedMap<Integer, VGDLSprite> getSprites(int spriteItype) {
   ...
   */
  public List<Effect> getCollisionEffects(int spriteItype1, int spriteItype2) {
   ...
   */
  public List<Pair> getDefinedEffects() {
   ...
   */
  public List<Integer> getDefinedEosEffects() {
   ...
   */
  public List<Effect> getEosEffects(int obj1) {
   ...
   */
  public Map<Character, ArrayList<String>> getCharMapping() {
    return charMapping;
  }
  ...
  public boolean isGameOver() {
    return Types.WINNER.NO_WINNER != winner;
  }
  ...
  protected static class Bucket {
    ...
    public int size(int bucket) {
      List<VGDLSprite> sprites = spriteLists.get(bucket);
      if (null == sprites)
        return 0;
      return sprites.size();
    }

    public Iterable<VGDLSprite> getAllSprites() {
      return allSprites;
    }

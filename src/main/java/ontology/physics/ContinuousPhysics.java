...
public class ContinuousPhysics extends GridPhysics {
	...
	public void passiveMovement(VGDLSprite sprite) {
		if (0 != sprite.speed && sprite.is_oriented) {
			sprite._updatePos(sprite.orientation, (int) sprite.speed);
			if (0 < gravity && 0 < sprite.mass) {
				Vector2d gravityAction = new Vector2d(0, gravity * sprite.mass);
	...
	public void activeMovement(VGDLSprite sprite, Vector2d action, double speed) {
		...
		double v1 = action.x / (float) sprite.mass + sprite.orientation.x
				* speed1;
		double v2 = action.y / (float) sprite.mass + sprite.orientation.y
				* speed1;
		Vector2d dir = new Vector2d(v1, v2);

		/*
		 * Vector2d unitDir = dir.unitVector(); sprite.orientation = unitDir;

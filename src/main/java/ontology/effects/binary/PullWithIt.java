...
/**
 * Created with IntelliJ IDEA. User: Diego Date: 04/11/13 Time: 15:56 This is a Java port from Tom Schaul's VGDL -
 * https://github.com/schaul/py-vgdl
 */
public class PullWithIt extends Effect {
  private int lastGameTime;

  private java.util.List<VGDLSprite> spritesThisCycle;
  ...
  public void execute(VGDLSprite sprite1, VGDLSprite sprite2, Game game) {
    // Keep in the list, for the current cycle, the sprites that have
    ...
    int gridsize = 1;
    if (Types.PHYSICS_GRID == sprite1.physicstype_id) {
      GridPhysics gp = (GridPhysics) sprite1.physics;
      gridsize = gp.gridsize.width;
    } else if (Types.PHYSICS_CONT == sprite1.physicstype_id) {
      GridPhysics gp = (GridPhysics) sprite1.physics;
      gridsize = gp.gridsize.width;
    }

    sprite1._updatePos(v, (int) (sprite2.speed * gridsize));
    if (Types.PHYSICS_CONT == sprite1.physicstype_id) {

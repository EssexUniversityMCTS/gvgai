...
public class Test {
	...
	public void testMain() {
		...
		String[] games = {"aliens", "boulderdash", "butterflies", "chase",
				"frogs", "missilecommand", "portals", "sokoban",
		...
		int gameIdx = 0;
		Paths.get(gamesPath.toString(), games[gameIdx] + ".txt").toString();
		int levelIdx = 0; // level names from 0 to 4 (game_lvlN.txt).
		String level1 = Paths.get(gamesPath.toString(),
				games[gameIdx] + "_lvl" + levelIdx + ".txt").toString();
		...
		for (int i = 0; i < N; ++i) {
			String game = Paths.get(gamesPath.toString(), ".",
					games[i] + ".txt").toString();
			int actionIdx = 0;
			for (int j = 0; j < L; ++j) {
				levels[j] = Paths.get(gamesPath.toString(),
						games[i] + "_lvl" + j + ".txt").toString();
				if (saveActions)
					for (int k = 0; k < M; ++k)

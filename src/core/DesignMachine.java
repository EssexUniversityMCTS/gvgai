...
public class DesignMachine {
    ...
    public static double[] runOneGame(String game_file, String level_file, boolean visuals, String agentNames, String actionFile, int randomSeed, int playerID) {
		...
		GameSpace toPlay = (GameSpace) (new VGDLParser().parseGame(game_file));
		toPlay.buildLevel(level_file, randomSeed);

		// Warm the game up.
		DesignMachine.warmUp(toPlay, CompetitionParameters.WARMUP_TIME);
		...
		for (int i = 0; i < no_players; i++) {
			...
			if (no_players > 1) {
			// multi player
			players[i] = DesignMachine.createMultiPlayer(names[i], actionFile, toPlay.getObservationMulti(),
				randomSeed, i, humans[i]);
			} else {
			// single player
			players[i] = DesignMachine.createPlayer(names[i], actionFile, toPlay.getObservation(), randomSeed,
				humans[i]);
			}
		...
		DesignMachine.tearPlayerDown(toPlay, players, actionFile, randomSeed, true);
...
//			if (no_players > 1) {
//			// multi player
//			players[i] = DesignMachine.createMultiPlayer(agentName, null, toPlay.getObservationMulti(), -1, i,
//				false);
//			} else {
//			// single player
//			players[i] = DesignMachine.createPlayer(agentName, null, toPlay.getObservation(), -1, false);
//			}
//
//			if (players[i] == null) {
//			// Something went wrong in the constructor, controller
...
//		DesignMachine.tearPlayerDown(toPlay, players, actionFile, seed, false);
//
...
//    }
    ...
    public static void runGames(String game_file, String[] level_files, int level_times, String agentName,
        ...
        for (String level_file : level_files) {
            ...
            for (int j = 0; j < no_players; j++) {
                if (no_players > 1) {
                // multi player
                players[j] = DesignMachine.createMultiPlayer(agentNames[j], filename,
                    toPlay.getObservationMulti(), randomSeed, j, false);
                } else {
                // single player
                players[j] = DesignMachine.createPlayer(agentNames[j], filename, toPlay.getObservation(),
                    randomSeed, false);
                }
                score[j] = -1;
                if (players[j] == null) {
            ...
            // down.
            if (!DesignMachine.tearPlayerDown(toPlay, players, filename, randomSeed, true)) {
                score = toPlay.handleResult();
                toPlay.printResult();
            }
     ...
     * @return Game created
     */
    public static GameSpace createGame(String game_file, String level_file, int randomSeed)
    {
        ...
        if (VERBOSE)
            System.out.println(" ** Creating game " + game_file + ", level " + level_file + " **");

...
public class OnlineMachine {
     ...
     */
    public double[] playOneGame(String game_file, String level_file, String actionFile, int randomSeed) {
     ...
     */
    public double[] runOneGame(String game_file, String level_file, boolean visuals, String agentNames,
        ...
        for (int i = 0; i < no_players; i++) {
            ...
            if (no_players > 1) {
                // multi player
                players[i] = ArcadeMachine.createMultiPlayer(names[i], actionFile, toPlay.getObservationMulti(0),
        ...
        double[] score;
        if (visuals) {
            this.dimension = toPlay.getScreenSize(); // TODO before starting the game can get the screen size here
            score = toPlay.playOnlineGame(this, players, randomSeed, anyHuman, playerID);
            //score = toPlay.playOnlineGame( this, players, randomSeed, anyHuman, playerID);
        }
        else
     ...
     *
     */
    public double[] replayOnlineGame(String game_file, String level_file, boolean visuals, String actionFile) {
        VGDLFactory.GetInstance().init(); // This always first thing to do.
        VGDLRegistry.GetInstance().init();
        ...
        for (int i = 0; i < no_players; i++) {
            if (no_players > 1) {
                // multi player
                players[i] = ArcadeMachine.createMultiPlayer(agentName, null, toPlay.getObservationMulti(0), -1, i,
                        false);
            } else {
                // single player
        ...
        try {
            ...
            if (no_players < 2) {
                ...
                informationText = "Replaying game in " + game_file + ", " + level_file + " with seed " + seed
        ...
        double[] score;
        if (visuals)
            //score = toPlay.playOnlineGame(this, players, seed, false, 0); // TODO initialize game
            score = toPlay.playOnlineGame(this, players, seed, false, 0);
        else

...
public class TestMultiPlayer {
	...
	// ArcadeMachine.runGames(game, new String[]{level1}, M, tracks,
	// null);
	// }
	...
	// + k + ".txt";
	// }
	// ArcadeMachine.runGames(game, levels, M, tracks, saveActions?
	// actionFiles:null);
	// }

	// 6. This plays a round robin style tournament between multiple
	// tracks, in N games, first L levels, M times each.
	// Controllers are swapped for each match as well. Actions to file
	...
	// String[] actionFiles = new String[L*M];
	//
	// String[] cont = new String[]{doNothingController, randomController,
	// oneStepController, sampleGAController,
	// sampleMCTSController, sampleOLMCTSController}; //add all tracks
	// that should play in this array
	//
	// for(int i = 0; i < N; ++i)
	// {
	// int actionIdx = 0;
	...
	// actionFiles[actionIdx++] = "actions_" + cont[k] + "_" + cont[t] +
	// "_game_" + i + "_level_" + j + "_" + p + ".txt";
	// }
	// tracks = cont[k] + " " + cont[t];
	...
	// for (int j = 0; j < L; ++j) {
	// for (int p = 0; p < M; ++p)
	// actionFiles[actionIdx++] = "actions_" + cont[t] + "_" + cont[k] +
	// "_game_" + i + "_level_" + j + "_" + p + ".txt";
	// }
	// }
	// tracks = cont[t] + " " + cont[k];
	// System.out.println(tracks);
	// ArcadeMachine.runGames(game, levels, M, tracks, saveActions ?
	// actionFiles : null);
	// }

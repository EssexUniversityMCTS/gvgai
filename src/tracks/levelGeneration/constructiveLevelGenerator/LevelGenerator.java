...
public class LevelGenerator extends AbstractLevelGenerator {
    ...
    /**
     * tradeoff between intended percentage of objects and the generated one
     * from the number of created objects
     */
    ...
    /**
     * Calculate the percentage covered from the level and percentages of each
     * data type
     * 
     * @param game
	...
	for (SpriteData sprite : game.getAllSpriteData()) {
	    ...
	    // number of solid objects (priority value, if not spawned and 0
	    // otherwise)
	    if (gameAnalyzer.getSolidSprites().contains(sprite.name)) {
		solidValue += value;
	    }
	    // number of harmful sprites (priority value, if not spawned and 0
	    // otherwise)
	    if (gameAnalyzer.getHarmfulSprites().contains(sprite.name)) {
		harmfulValue += value;
	    }
	    // number of other sprites (priority value, if not spawned and 0
	    // otherwise)
	    if (gameAnalyzer.getOtherSprites().contains(sprite.name)) {
		otherValue += value;
	    }
	    // number of collectable sprites (priority value, if not spawned and
	    // 0 otherwise)
	...
	// covered percentage is inversely proportional with number of spawned
	// objects
	// and harmful objects but directly proportional with collectable
	// objects
	...
	// if the avatar is horizontal mover or shooter it placed at the top or
	// the bottom
	// free positions
	...
	for (TerminationData ter : termination) {
	    ...
	    // difference between the expected number and the current number of
	    // objects
	    increase = totalNum + 1 - currentNum;

	    // if the condition is satisfied add more sprites to make till be
	    // unstaisfied
	...
	while (numberOfHarmful > 0) {
	    ...
	    // if the harmful object is moving then get a far position from the
	...
	int width = (int) Math.max(minSize + size,
		game.getAllSpriteData().size() * ((levelSizeMaxPercentage - levelSizeRandomPercentage)
			+ levelSizeRandomPercentage * random.nextDouble()) + size);
	int length = (int) Math.max(minSize + size,
		game.getAllSpriteData().size() * ((levelSizeMaxPercentage - levelSizeRandomPercentage)
			+ levelSizeRandomPercentage * random.nextDouble()) + size);

...
java -cp ${server_jar} -agentlib:jdwp=transport=dt_socket,server=y,address=8888,suspend=n core.competition.LearnerExecutor -gameId ${game_id} -gameFile ${game_file} -levelFile ${level_file} -gamesDir ${games_prefix} -imgDir ${img_dir} > ${log_dir}/output_server_redirect_game${game_id}.txt 2> ${log_dir}/output_server_redirect_err_game${game_id}.txt
